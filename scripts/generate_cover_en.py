...
def generate_cover_en():
    ...
    # 1. Inpaint Russian title area: y: 160..250, x: 200..810
    top_clean = arr[155:160, 200:810].mean(axis=0)
    bot_clean = arr[250:255, 200:810].mean(axis=0)
    np.random.seed(42)
    for y in range(160, 250):
        t = (y - 160) / (250 - 160)
        noise = np.random.normal(0, 0.6, (610, 4))
        arr[y, 200:810] = (1 - t) * top_clean + t * bot_clean + noise
    ...
    for y in range(800, 855):
        t = (y - 800) / (855 - 800)
        noise = np.random.normal(0, 0.6, (260, 4))
    ...
    font_top = ImageFont.truetype(font_path_bold, 48)
    font_bot = ImageFont.truetype(font_path_bold, 28)

    # Top title: "POINT OF SUPPORT" strictly fitted inside book bounds
    draw_spaced_text(draw, 'POINT OF SUPPORT', font_top, 512, 203, (255, 255, 255, 255), letter_spacing=2)
    # Bottom author: "MAX" perfectly centered and styled
    draw_spaced_text(draw, 'M A X', font_bot, 512, 825, (160, 172, 190, 255), letter_spacing=18)
